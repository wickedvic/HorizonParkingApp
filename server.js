...
//   const query = `
//     SELECT c.*, 
//     MAX(CASE WHEN p.end_date >= $1 THEN 1 ELSE 0 END) as has_active_permit
//     FROM clients c
//     LEFT JOIN cars car ON c.id = car.client_id
//     LEFT JOIN permits p ON car.id = p.car_id
//     GROUP BY c.id
...
//   try {
//     const result = await pool.query(
//       "INSERT INTO clients (first_name, last_name, email, phone, client_type) VALUES ($1, $2, $3, $4, $5) RETURNING id",
//       [first_name, last_name, email, phone, client_type || 'temp']
//     );
//     res.json({ id: result.rows[0].id, first_name, last_name, email, phone, client_type });
//   } catch (err) {
...
//   const { active } = req.body;
//   try {
//     await pool.query("UPDATE clients SET active = $1 WHERE id = $2", [active, req.params.id]);
//     res.json({ success: true });
//   } catch (err) {
...
//   let query = `
//     SELECT c.*, cl.first_name, cl.last_name,
//     (SELECT COUNT(*) FROM payments p WHERE p.client_id = c.client_id AND p.is_paid = FALSE) as outstanding_payments,
//     (SELECT COUNT(*) FROM permits p WHERE p.car_id = c.id AND p.end_date >= $1) as has_active_permit
//     FROM cars c 
//     JOIN clients cl ON c.client_id = cl.id 
//   `;
//   const params = [today];
//   if (clientId) {
//     query += ` WHERE c.client_id = $2`;
//     params.push(clientId);
//   }
//   query += ` ORDER BY c.license_plate`;
//   try {
...
//   try {
//     const result = await pool.query(
//       "INSERT INTO cars (client_id, license_plate, make, model, color, year) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id",
//       [client_id, license_plate, make, model, color, year]
//     );
//     res.json({ id: result.rows[0].id, client_id, license_plate, make, model, color, year });
//   } catch (err) {
...
//       "INSERT INTO permits (permit_number, car_id, permit_type, start_date, end_date, daily_rate, total_cost) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id",
//       [permit_number, car_id, permit_type, start_date, end_date, daily_rate, total_cost]
//     );
//     const newPermitId = permitResult.rows[0].id;

//     const carResult = await pool.query("SELECT client_id FROM cars WHERE id = $1", [car_id]);
//     if (carResult.rows[0]) {
//       await pool.query(
//         "INSERT INTO payments (permit_id, client_id, amount, is_paid) VALUES ($1, $2, $3, FALSE)",
//         [newPermitId, carResult.rows[0].client_id, initialPayment]
//       );
//     }
//     res.json({ id: newPermitId, ...req.body });
//   } catch (err) {
...
//   try {
//     await pool.query(
//       "UPDATE payments SET is_paid = $1, paid_date = $2 WHERE id = $3",
//       [is_paid, is_paid ? (paid_date || new Date().toISOString()) : null, req.params.id]
//     );
//     res.json({ success: true });
//   } catch (err) {
...
// // --- MONTHLY GENERATION (MODERNIZED) ---
// app.post("/api/payments/generate-monthly", async (req, res) => {
//   const currentMonth = new Date().toISOString().slice(0, 7);
//   const today = new Date().toISOString().split("T")[0];
  
//   try {
//     const monthlyPermits = await pool.query(`
//       SELECT p.*, c.client_id 
//       FROM permits p 
//       JOIN cars c ON p.car_id = c.id 
//       JOIN clients cl ON c.client_id = cl.id
//       WHERE p.permit_type = 'monthly' AND cl.active = TRUE
//     `);

//     let createdCount = 0;
//     let extendedCount = 0;

//     for (const permit of monthlyPermits.rows) {
//       // Auto-Extend if expired
//       if (permit.end_date < today) {
//         const newEndDate = new Date();
//         newEndDate.setDate(new Date(today).getDate() + 30);
//         const newTotal = parseFloat(permit.total_cost) + parseFloat(permit.daily_rate);
        
//         await pool.query(
//           "UPDATE permits SET end_date = $1, total_cost = $2 WHERE id = $3",
//           [newEndDate.toISOString().split('T')[0], newTotal, permit.id]
//         );
//         permit.total_cost = newTotal; // Update local reference for calculation below
//         extendedCount++;
//       }

//       // Check existing payments
//       const paymentStatus = await pool.query(
//         "SELECT SUM(amount) as total_paid FROM payments WHERE permit_id = $1",
//         [permit.id]
//       );
//       const totalPaid = parseFloat(paymentStatus.rows[0].total_paid || 0);

//       if (totalPaid < permit.total_cost) {
//         // Check if billed this month already
//         const alreadyBilled = await pool.query(
//           "SELECT id FROM payments WHERE permit_id = $1 AND TO_CHAR(created_at, 'YYYY-MM') = $2",
//           [permit.id, currentMonth]
//         );

//         if (alreadyBilled.rows.length === 0) {
//           const amountToBill = Math.min(permit.daily_rate, permit.total_cost - totalPaid);
//           await pool.query(
//             "INSERT INTO payments (permit_id, client_id, amount, is_paid) VALUES ($1, $2, $3, FALSE)",
//             [permit.id, permit.client_id, amountToBill]
//           );
...
//       }
//     }
//     res.json({ success: true, createdCount, extendedCount });
//   } catch (err) {
...
//   let query = `
//     SELECT 
//       cl.id, cl.first_name, cl.last_name, cl.email, cl.client_type,
//       COUNT(DISTINCT c.id) as car_count,
//       COUNT(DISTINCT per.id) as permit_count,
//       COALESCE(SUM(CASE WHEN pay.is_paid = TRUE THEN pay.amount ELSE 0 END), 0) as total_paid,
//       COALESCE(SUM(CASE WHEN pay.is_paid = FALSE THEN pay.amount ELSE 0 END), 0) as total_pending
//     FROM clients cl
//     LEFT JOIN cars c ON cl.id = c.client_id
//     LEFT JOIN permits per ON c.id = per.car_id
//     LEFT JOIN payments pay ON per.id = pay.permit_id`;
...
//   const params = [];
//   if (startDate && endDate) {
//     query += ` WHERE per.start_date >= $1 AND per.start_date <= $2`;
//     params.push(startDate, endDate);
//   }
//   query += ` GROUP BY cl.id ORDER BY cl.first_name`;
